...
shopt -s histappend
# Removed "history -c" and "history -r" as it causes other windows cmds to immediately show up in the current window which is annoying. With this setup commands from other windows are only added to the global history when the session is killed. Every new window will then have access to that history
...
export GIT_EDITOR="neovim/build/bin/nvim"
